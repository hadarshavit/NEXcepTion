...
def main():
    ...
    if args.log_wandb:
        if has_wandb:
            wandb.init(project=args.experiment, config=args, entity='autoconvnext')
        else: 
    ...
    if args.model == 'nexception-t':
        from nexception_t import NEXception
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank], broadcast_buffers=not args.no_ddp_bb, static_graph=True)
